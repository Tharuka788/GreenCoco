...
const EditEmployee = () => {
  ...
  const validate = () => {
    ...
    if (!employee.EmployeeName.trim()) {
      ...
      tempErrors.EmployeeName = 'Employee Name must contain only letters and spaces';
    }

    // Validate Department Name
    if (!employee.DepartmentName.trim()) {
      tempErrors.DepartmentName = 'Department Name is required';
    } else if (!/^[a-zA-Z\s]+$/.test(employee.DepartmentName)) {
      tempErrors.DepartmentName = 'Department Name must contain only letters and spaces';
    }
    ...
    if (!employee.JobRole.trim()) {
      ...
      tempErrors.JobRole = 'Job Role must contain only letters and spaces';
    }
    ...
    else if (!/^\d{10}$/.test(employee.PhoneNumber)) tempErrors.PhoneNumber = 'Phone Number must be exactly 10 digits';
